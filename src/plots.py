...
All functions accept pre-computed arrays and a ``save_path`` argument.
They do NOT call ``plt.show()`` so they are safe for headless CI/CD
environments.

Compatibility note
──────────────────
RocCurveDisplay.from_predictions() and PrecisionRecallDisplay.from_predictions()
do not accept a ``color`` kwarg in scikit-learn >= 1.4.
Colour is applied post-draw via _apply_colour_to_last_line().
"""
...
# Consistent colour palette across all single-model and comparison plots
...
def _apply_colour_to_last_line(ax: plt.Axes, colour: str) -> None:
    """
    Apply ``colour`` to the most recently drawn line on ``ax``.

    Required because RocCurveDisplay.from_predictions() and
    PrecisionRecallDisplay.from_predictions() do not accept a ``color``
    keyword argument in scikit-learn >= 1.4.

    Parameters
    ----------
    ax : plt.Axes
        The axes containing the line to recolour.
    colour : str
        Any matplotlib-compatible colour string (hex, named, etc.).
    """
    lines = ax.get_lines()
    if lines:
        lines[-1].set_color(colour)
...
def plot_roc_curve(
    ...
    """
    Generate and save a single-model ROC curve figure.
    ...
    roc_auc : float
        Pre-computed ROC-AUC score for legend annotation.
    save_path : Path
        Destination file path.
    model_name : str
        Model display name used in the legend and title.
    """
    ...
    with plt.style.context(_STYLE):
        ...
        _apply_colour_to_last_line(ax, colour)
...
def plot_pr_curve(
    ...
    """
    Generate and save a single-model Precision-Recall curve figure.
    ...
    y_prob : np.ndarray
        Predicted positive-class probabilities.
    pr_auc : float
        Pre-computed PR-AUC score for legend annotation.
    save_path : Path
        Destination file path.
    model_name : str
        Model display name used in the legend and title.
...
def plot_confusion_matrix(
    ...
    y_true : np.ndarray
        Ground-truth binary labels.
    y_pred : np.ndarray
        Binary predictions at the decision threshold.
    save_path : Path
        Destination file path.
    model_name : str
        Model display name used in the plot title.
    """
    with plt.style.context(_STYLE):
...
def plot_feature_importance(
    ...
    importances : np.ndarray
        Gini / MDI importance values from a tree-based model.
    top_n : int
        Number of top features to display (default 20).
    save_path : Path | None
        If provided, saves the figure; otherwise calls plt.show().
    model_name : str
        Model display name used in the plot title.
    """
...
def plot_metrics_comparison_bar(
    ...
    all_metrics : dict[str, dict]
        ...
        Each metrics_dict must contain keys:
